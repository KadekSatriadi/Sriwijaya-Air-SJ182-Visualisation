    ...
    */
    ...
    void LateUpdate()
    {
        ...
        if (re)
        {
            ...
            if (colors.Length == meshVertices.Length)
                mesh.colors = colors;
            mesh.triangles = tris;
    ...
    public void SetPoints(Vector3[] points, float radius, Color[] col)
    {
        ...
        vertices[0] = new TubeVertex(v0offset + points[0], 0.0f, col[0]);
        Vector3 v1offset = (points[points.Length - 1] - points[points.Length - 2]) * 0.01f;
        vertices[vertices.Length - 1] = new TubeVertex(v1offset + points[points.Length - 1], 0.0f, col[points.Length - 1]);

        for (int p = 0; p < points.Length; p++)
        {
            vertices[p + 1] = new TubeVertex(points[p], radius, col[p]);
        }
    }
}
